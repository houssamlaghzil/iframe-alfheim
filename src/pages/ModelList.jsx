...
export default function ModelList() {
    ...
    return (
        <main className="page max-w-7xl mx-auto px-4 py-12">
            ...
            {envs.length === 0 ? (
                <p className="text-gray-400">Aucun environnement enregistré.</p>
            ) : (
                <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {envs.map(env => (
                        <Link key={env.id} to={`/model/${env.id}`} className="card hover:shadow-lg transition">
                            <div className="h-44 bg-gradient-to-br from-[#2e2e57] to-[#1c1c38] flex items-center justify-center">
                            ...
                            <div className="p-4 space-y-1 bg-white">
                                ...
                                <p className="text-sm text-gray-500 line-clamp-2">{env.subtitle}</p>
                            </div>
                        </Link>
                    ))}
                </div>
